...
class NuScenesReachMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        self.enable_lanediffusion = enable_lanediffusion
        self.lanediff_config = lanediff_config
    ...
    def _evaluate_single(
        ...
        metrics = RoadnetReachDistEval(result_path=result_path, 
                                           ...
                                           enable_lanediffusion=self.enable_lanediffusion,
                                           lanediff_config=self.lanediff_config
                                           )
        ...
        # LaneDiffusion Metrics Integration
        if hasattr(self, 'enable_lanediffusion') and self.enable_lanediffusion:
            if logger:
                logger.info("Calculating LaneDiffusion metrics...")
            
            # 获取几何参数
            from projects.SeqGrowGraph.seq_grow_graph.transforms.roadnet_reach_dist_eval_new import get_geom
            dx, bx, nx, pc_range, _ = get_geom(self.grid_conf)
            
            # 这里的metrics字典中应该包含EvalSeq2Graph对象列表
            # 但目前的RoadnetReachDistEval只返回了数值结果
            # 我们需要修改RoadnetReachDistEval使其返回图对象，或者在这里重新加载/构建
            # 考虑到RoadnetReachDistEval使用多进程且逻辑复杂，重新构建可能更安全但效率低
            # 更好的方式是让RoadnetReachDistEval返回转换后的networkx图列表（如果可能）
            # 或者，我们可以直接在这里使用HybridEvaluator的evaluate_lanediffusion_batch
            # 但我们需要GT和Pred的EvalSeq2Graph对象列表
            
            # 由于RoadnetReachDistEval内部已经加载了数据并进行了转换
            # 我们尝试从metrics中获取（如果我们在RoadnetReachDistEval中添加了返回）
            # 或者我们在这里重新加载数据（会有重复开销）
            
            # 方案B：在RoadnetReachDistEval内部集成（更高效，避免重复加载）
            # 但用户要求在nus_reach_metric.py中集成
            # 让我们假设RoadnetReachDistEval已经修改为返回'lanediff_metrics'（如果我们在那里集成）
            # 或者我们在这里重新加载数据
            
            # 鉴于RoadnetReachDistEval非常复杂且使用了多进程，
            # 最稳妥的方式是在RoadnetReachDistEval内部集成，然后在这里提取结果
            
            if 'lanediff_metrics' in metrics:
                lanediff_metrics = metrics['lanediff_metrics']
                for key, value in lanediff_metrics.items():
                    detail[f'{metric_prefix}/{key}'] = float('{:.4f}'.format(value))
            else:
                if logger:
                    logger.warning("LaneDiffusion metrics enabled but not returned by RoadnetReachDistEval")
