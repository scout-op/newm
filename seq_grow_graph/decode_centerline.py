...
# 从预测序列重建图结构
# Token分类：
# split_node=572：分隔不同节点
# split_connect=571：分隔"from"和"to"关系
# end=573：序列结束
# start=574：序列开始
class EvalSeq2Graph(EvalMapBzGraph):
    ...
    def render_image(self, nx, scale=5, res=None, bitmap=None):
        """Render visualization image and return it (without saving).
        
        Args:
            nx: Image size [width, height]
            scale: Scaling factor for visualization
            res: Optional result text to display
            bitmap: Optional background image
            
        Returns:
            image: Rendered image as numpy array
        """
        if bitmap is not None:
            image = bitmap
            image = cv2.resize(image, (int(nx[0]) * scale, int(nx[1]) * scale))
    ...
    def visualization(self, nx, path, aux_name, name, scale=5, res=None, bitmap=None):
        """Render and save visualization image."""
        image = self.render_image(nx, scale, res, bitmap)
